  ...
  async confirmPayment(userId: string, recordId: string, actualAmount?: number, actualDate?: Date) {
    ...
    if (subscription) {
        ...
        if (subscription.autoRenewal) {
          ...
          // Keep status='ACTIVE' — the subscription is still in its coverage period.
          // When nextPayment (expiry date) arrives, the daily cron job will detect
          // autoRenewal=false and set status='Expired' (instead of generating a new bill).
          await this.advanceSubscriptionDate(subscription);
        }
