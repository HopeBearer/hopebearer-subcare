  ...
  async confirmPayment(userId: string, recordId: string, actualAmount?: number, actualDate?: Date) {
    ...
    if (subscription) {
        ...
        if (subscription.autoRenewal) {
          // Auto-renewal on: advance to next cycle and potentially generate next bill
          ...
          // Auto-renewal off: advance nextPayment to represent the "expiry date"
          // (end of current billing period). The user just paid for this cycle,
          // so the subscription is valid until the next cycle date.
          // Set status='Expired' so no more bills will be generated by Cron
          // (findDueSubscriptions checks status='ACTIVE').
          await this.advanceSubscriptionDate(subscription);
          const advancedSub = await this.subscriptionRepository.findById(subscription.id);
          await this.subscriptionRepository.update(subscription.id, {
            status: 'Expired',
            // Keep the advanced nextPayment as the expiry date for display
            nextPayment: advancedSub?.nextPayment ?? null
          });
        }

        // Budget check runs regardless of autoRenewal status
        const freshSub = await this.subscriptionRepository.findById(subscription.id);
        if (freshSub) {
