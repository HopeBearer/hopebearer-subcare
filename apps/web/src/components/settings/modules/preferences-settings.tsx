...
export function PreferencesSettings() {
  ...
  const { data: supportedCurrencies = [], isLoading: isLoadingCurrencies } = useQuery({
    queryKey: ['currencies'],
  ...
  const currencyOptions = supportedCurrencies.map(c => ({
    label: c, // Backend currently returns string[]. Ideally it could return {code, name}
    value: c
  }));
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      // Fetch preview from backend using NEW Currency Service
      // "previewConversion" was likely a mock or old logic. 
      // We use the real convert API: /api/v1/currency/convert
      const conversionResult = await currencyService.convert(oldBudget, oldCurrency, currency);

      // result might be an object or number depending on API. 
      // Backend: data: { amount, from, to, result }
      setConvertedBudget(conversionResult.result);
      ...
      <Card className="p-6 space-y-6">
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <Button onClick={handleSave} disabled={loading || isLoadingCurrencies} className="px-4 py-2 text-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Select
              ...
              options={currencyOptions.length > 0 ? currencyOptions : [{ label: 'Loading...', value: '' }]}
